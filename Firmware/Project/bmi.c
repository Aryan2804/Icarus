...
static gyro_read (uint8_t reg_addr, uint8_t *data, size_t len){
    uint8_t tx[9] = {0};
    uint8_t rx[9] = {0};
...
static accel_read (uint8_t reg_addr, uint8_t *data, size_t len){
    uint8_t tx[10] = {0};
    uint8_t rx[10] = {0};
    tx[0] = reg_addr | 0x80
    ...
    esp_err_t ret = spi_device_transmit(aaccel_spi, &t);         // Toggles the accel CS pin
    if (ret == ESP_OK){                                          // If no error, then it starts copying the sensor data           
